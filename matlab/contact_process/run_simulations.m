...
local_sim_independent_data = zeros(num_iterations*t_end, 3);
index = 1;
for iter = 1:num_iterations
    S = local_approx_independent(3,t_end, p, q);
    local_sim_independent_data(index:index+t_end-1,:) = S;
    index = index + t_end;
    disp(iter);
end
save('local_sim_independent_data.mat', 'local_sim_independent_data')
